...
class Subscribers
{
    ...
    public static function subscribersGet($order_by = 'id', $order = 'asc', $limit = 100, $offset = 0)
    {
        ...
        /* ------------- Get subscribers available attributes ------------- */
        $sql        = "SELECT * FROM ".$GLOBALS['tables']['attribute'].";";
        $stmt       = $db->prepare($sql);
        $stmt->execute();
        $attributes             = $stmt->fetchAll(PDO::FETCH_ASSOC);

        foreach($attributes as $attr){
            $avAttrs[ $attr['id']] = $attr['tablename'];
        }
        ...
        foreach($contacts as $contact){
            ...
            $sql        = "SELECT * FROM ".$GLOBALS['tables']['user_attribute']." WHERE userid=:userid;";
            $stmt       = $db->prepare($sql);
    
            $stmt->bindParam('userid',       $contact['id'],    PDO::PARAM_INT);
            $stmt->execute();
            $usrAttr                = $stmt->fetchAll(PDO::FETCH_ASSOC);

            /* ------------- Map subscriber attributes ------------- */
            foreach($usrAttr as $attr){
                $contact['attributes'][$avAttrs[$attr['attributeid']]] = $attr['value'];
            }
        ...
        $response->setData('subscribers', $arr);     
        $response->output();
    }
    ...
    /** 
     * Get all subscriber available attributes
     */
    public static function subscriberGetAttributes() {
        Common::select('Attribute', 'SELECT * FROM '.$GLOBALS['tables']['attribute'], false);
    }
